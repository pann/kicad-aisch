...
The `kicad-schematic` skill guides you through designing and generating production-ready KiCad 9 hierarchical schematics from a plain-text design description. It follows a structured 7-phase workflow:
...
5. **BOM** — compile bill of materials, check KiCad library availability
6. **KiCad Project** — set up project, import components, create hierarchy
7. **Schematic Generation** — generate Python scripts that produce `.kicad_sch` files
...
- **Output:** `workflow/05-bom.md` + `workflow/05-bom.csv`
- **What to review:** All components listed? KiCad library status correct? Import list complete?
...
- **Output:** `scripts/gen_*.py` generator scripts + `.kicad_sch` schematic files
- **What to review:** ERC clean (0 errors)? SVG exports readable? Labels don't overlap? Wiring correct?
